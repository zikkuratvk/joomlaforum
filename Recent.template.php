...
function template_unread()
{
	...
	if ($settings['show_mark_read'])
	{
		...
		$mark_read = array(
			'check-circle' => array('text' => !empty($context['no_board_limits']) ? 'mark_as_read' : 'mark_read_short', 'image' => 'markread.gif', 'lang' => true, 'url' => $scripturl . '?action=markasread;sa=' . (!empty($context['no_board_limits']) ? 'all' : 'board' . $context['querystring_board_limits']) . ';' . $context['session_var'] . '=' . $context['session_id']),
		);
	...
	if (!empty($context['topics']))
	{
				...
				<table class="table_grid" cellspacing="0">
					<thead>
						<tr class="catbg">
							...
							<th scope="col" class="first_mobile">
							...
							<th scope="col" width="14%" class="hidden-xs" align="center">
		...
		if (!empty($context['topics']) && !$context['showing_all_topics'])
			$mark_read['angle-double-right'] = array('text' => 'unread_topics_all', 'image' => 'markreadall.gif', 'lang' => true, 'url' => $scripturl . '?action=unread;all' . $context['querystring_board_limits'], 'active' => true);
...
function template_replies()
{
	...
	if (isset($context['topics_to_mark']) && !empty($settings['show_mark_read']))
	{
		// Generate the button strip.
		$mark_read = array(
			'check-circle' => array('text' => 'mark_as_read', 'image' => 'markread.gif', 'lang' => true, 'url' => $scripturl . '?action=markasread;sa=unreadreplies;topics=' . $context['topics_to_mark'] . ';' . $context['session_var'] . '=' . $context['session_id']),
		);
